...
import com.ssk.hfb.common.pojo.Payload;
...
public class JwtUtils {
    ...
    private static final String JWT_PAYLOAD_USER_KEY = "@lhb.com9527";
    private static final int EXPIRE_TIME = 120; //过期时间，单位分钟
    /**
     * 私钥加密token
     *
    ...
    public static UserInfo getInfoFromToken(String token, PublicKey publicKey) {
        Jws<Claims> claimsJws = parserToken(token, publicKey);
        Claims body = claimsJws.getBody();
        UserInfo info = JsonUtils.toBean(body.get(JWT_PAYLOAD_USER_KEY).toString(), UserInfo.class);
        return info;
    }
